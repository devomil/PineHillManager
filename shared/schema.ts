...
// Document Categories - For organizing documents in the Document Center
export const documentCategories = pgTable("document_categories", {
  ...
  nameIdx: index("idx_document_categories_name").on(table.name),
}));
...
// Center Documents - Files, images, and documents in the Document Center
export const centerDocuments = pgTable("center_documents", {
  id: serial("id").primaryKey(),
  title: varchar("title", { length: 500 }).notNull(),
  description: text("description"),
  ...
  categoryIdx: index("idx_center_documents_category_id").on(table.categoryId),
  uploadedByIdx: index("idx_center_documents_uploaded_by").on(table.uploadedBy),
  titleIdx: index("idx_center_documents_title").on(table.title),
}));

export const insertCenterDocumentSchema = createInsertSchema(centerDocuments).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export type CenterDocument = typeof centerDocuments.$inferSelect;
export type InsertCenterDocument = z.infer<typeof insertCenterDocumentSchema>;
