...
export default function TimeClock() {
  ...
  const calculateTodaysHours = () => {
    if (!todayEntries.length) return "0:00";
  ...
  const calculateWeekHours = () => {
    if (!weekEntries.length) return "0:00";
  ...
  const calculateCurrentBreakTime = () => {
    if (!currentEntry || currentEntry.status !== 'on_break' || !currentEntry.breakStartTime) {
      return "0:00";
    }
  ...
  const isClockedIn = currentEntry?.status === 'clocked_in' || currentEntry?.status === 'on_break';
