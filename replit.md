...
- **Inventory Management**: Dedicated `/inventory` page with comprehensive tracking, real-time stock levels, categories, and analytics across all locations. Fully synced with Clover POS system (5,981 items across 5 locations). Location tracking implemented via `locationId` mapping to Clover config IDs. Dashboard excludes negative stock items (primarily BioScan services) from value calculations to ensure accurate inventory valuation. Out of 5,981 items, 143 have active stock (2.4%). Last synced: October 8, 2025.
