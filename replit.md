...
- **Inventory Management**: Dedicated `/inventory` page with comprehensive tracking, real-time stock levels, categories, vendor analytics, and cross-platform analytics across all locations. Fully synced with Clover POS system (5,981 items across 5 locations). Location tracking implemented via `locationId` mapping to Clover config IDs. Dashboard excludes negative stock items (primarily BioScan services) from value calculations to ensure accurate inventory valuation. Out of 5,981 items, 143 have active stock (2.4%). Vendor analytics tab aggregates inventory by supplier with total value, item count, and location-specific breakdowns. CSV import tool enables bulk vendor data updates from Clover inventory reports. Last synced: October 9, 2025.
