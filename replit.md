...
- **Employee Purchase Portal**: Dedicated `/employee-purchases` page for employees to scan and purchase inventory items using their monthly allowance, with real-time balance tracking and purchase history. Pricing model: full retail price charged against free monthly allowance (before cap), then cost+markup employee discount pricing for out-of-pocket purchases (after exceeding cap). Managers and admins receive 100% discount (COG pricing only) for all purchases. Admin reporting available at `/admin/employees/purchases` showing spending, allowances, and cost markup percentages across all employees.
