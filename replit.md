...
- **Inventory Management**: Dedicated `/inventory` page with comprehensive dual-system tracking reconciling Clover POS (live inventory) and Thrive (vendor/cost data). Features include:
  ...
  - **Manual Matching API** (Admin-only): POST `/api/accounting/inventory/manual-match` links unmatched Thrive items to Clover items. GET `/api/accounting/inventory/match-suggestions/:thriveItemId` provides smart suggestions with multi-factor scoring (SKU exact match: 100, name exact: 80, partial name: 50, word overlap: 10/word, category: 20).
